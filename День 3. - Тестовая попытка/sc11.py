...
class TargetTracker:
    def __init__(self):
        ...
        # Управление
        self.kp_yaw = 0.0007     # очень медленный поворот
        self.kp_dist = 0.8
        self.max_speed_x = 0.3
        self.center_threshold = 50

        # Состояние — сразу TRACK
        self.state = 'TRACK'

        # Ограничение yaw
        self.prev_yaw = 0.0
        self.yaw_rate_limit = 0.01
    ...
    def smooth_yaw(self, yaw):
        ...
        dy = max(-self.yaw_rate_limit, min(self.yaw_rate_limit, dy))
        yaw = self.prev_yaw + dy
        self.prev_yaw = yaw
        return yaw
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            ...
            # --- если цель потеряна ---
            if target is None:
                rospy.logwarn_throttle(2, "Цель потеряна — зависаем")
                self.send_cmd(0.0, 0.0)
                rate.sleep()
                continue

            # --- центр по чёрному или bbox ---
            ...
            rospy.loginfo_throttle(1,
                "dist=%.2f err=%.0f yaw=%.3f move_x=%.2f",
